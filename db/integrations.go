...
// SetFriendsG removes all previously related items of the
// integration replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Integration's Friends accordingly.
// Replaces o.R.Friends with related.
// Sets related.R.Integration's Friends accordingly.
// Uses the global database handle.
func (o *Integration) SetFriendsG(insert bool, related ...*Friend) error {
	return o.SetFriends(boil.GetDB(), insert, related...)
}

// SetFriends removes all previously related items of the
// integration replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Integration's Friends accordingly.
// Replaces o.R.Friends with related.
// Sets related.R.Integration's Friends accordingly.
func (o *Integration) SetFriends(exec boil.Executor, insert bool, related ...*Friend) error {
	query := "update \"friends\" set \"integration_id\" = null where \"integration_id\" = ?"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	return o.AddFriends(exec, insert, related...)
}

// RemoveFriendsG relationships from objects passed in.
// Removes related items from R.Friends (uses pointer comparison, removal does not keep order)
// Sets related.R.Integration.
// Uses the global database handle.
func (o *Integration) RemoveFriendsG(related ...*Friend) error {
	return o.RemoveFriends(boil.GetDB(), related...)
}

// RemoveFriends relationships from objects passed in.
// Removes related items from R.Friends (uses pointer comparison, removal does not keep order)
// Sets related.R.Integration.
func (o *Integration) RemoveFriends(exec boil.Executor, related ...*Friend) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.IntegrationID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.Friends {
			...
			ln := len(o.R.Friends)
			if ln > 1 && i < ln-1 {
				o.R.Friends[i] = o.R.Friends[ln-1]
			}
			o.R.Friends = o.R.Friends[:ln-1]
			break
		}
	}
